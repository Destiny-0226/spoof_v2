...
来自宿主 SMBIOS，再从对应容量池选择真实料号。Guest 内存按最大常见单条规格
优先拆分，单条最小 4 GiB，例如 8 GiB 为 `1x8`、12 GiB 为 `8+4`，不会用
`3x4` 凑数。Type 16/17/19/20 只描述 Guest 实际容量和这些虚拟 DIMM，不复制
宿主物理内存容量或已安装条数。I226-V 是统一的独立 PCIe 网卡。
...
身份文件当前 schema 为 23，宿主平台来源版本为 3。修改或重新运行 `01` 后，必须按顺序重新
