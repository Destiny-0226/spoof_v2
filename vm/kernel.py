...
def _validate_kernel_source(profile: KernelProfile, source: Path) -> None:
    ...
    if source_tag is None:
        raise AppError(
            "7.2 实验 profile 需要可辨识的固定 release tag source；"
            f"实际版本无法辨识：{source}"
        )
    if source_tag != profile.tkg_version:
        raise AppError(
            f"7.2 实验 profile 固定使用 {profile.tkg_version}，收到 {source_tag}：{source}"
        )
    tag = ""
    ...
    if tag and tag != source_tag:
        raise AppError(
            f"7.2 source 的 git tag ({tag}) 与 Makefile 版本 ({source_tag}) 不一致：{source}"
        )
...
def _kernel_patch(profile: KernelProfile) -> Path:
    ...
    if not patch.is_file():
        if profile.test_release:
            raise AppError(
                f"{profile.label} 尚未提供 CPU 对应补丁：{patch.name}。"
                "不能把 6.19 补丁直接套到 7.2；请先完成该版本的 port。"
            )
        raise AppError(f"找不到自订核心补丁：{patch}")
    return patch
...
def _validate_hypercall_patch(patch: Path) -> None:
    ...
    if "SVM_EXIT_VMMCALL" in text:
        ...
        raise AppError(
            f"核心补丁缺少 VMCALL/VMMCALL exit handler：{patch.name}。"
        )
    handler = re.compile(
    ...
    if handler.search(text) is None:
        raise AppError(
            f"核心补丁 {patch.name} 未将 {marker} 导向 kvm_handle_invalid_op；"
            "拒绝建置会暴露 KVM hypercall interception 的核心。"
        )
...
def _validate_amd_cpuid_virtualization_patch(patch: Path) -> None:
    ...
    if len(recalc_gated) != 1 or len(init_gated) != 1 or not sticky_arm:
        problems.append(
            "必须在 recalc/init_vmcb 使用 EFER.SVME 加 sticky arm 控制 VMCB01 CPUID intercept"
        )
    if not delayed_vmcbo1_handoff or len(nested_arm_sev_safe) != 1:
        problems.append(
            "必须保留以 jiffies reset grace 驱动、排除 SEV/L2 的 VMCB01 CPUID handoff"
        )
    if len(nested_l1_efer) != 1:
        problems.append("nested active 时必须使用 VMCB01 保存的 L1 EFER.SVME")
    if len(nested_leaf0) != 1:
        problems.append("必须保留 nested L2 CPUID leaf 0 的 L0 fallback handler")
    if len(nested_leaf0_fastpath) != 1:
        problems.append("必须以 PMU/CPUID-fault safe IRQ-off fastpath 处理 nested leaf 0")
    if len(nested_leaf0_cache) != 1:
        problems.append("必须在 set_cpuid 后预先快取 leaf 0，不得在 IRQ-off 路径查表")
    if not nested_leaf0_short_reentry:
        problems.append("必须保留 request boundary 的 SVM nested leaf-0 short re-entry")
    if len(nested_leaf0_direct_skip) != 1:
        problems.append("nested leaf 0 必须以 TF/PMU guard 直接提交 NRIPS 与 interrupt-shadow")
    if not nested_leaf0_deferred_tail:
        problems.append("必须保留事件/PMU/TLB guard 与完整状态回存的 leaf-0 deferred exit-tail")
    if not amd_cpuid_signature_mask:
        problems.append("AMD guest 的 CPUID leaf 7 EDX 必须清除 Intel mitigation bits 26/27/31")
    if len(nested_db_direct_reflection) != 1:
        problems.append("必须保留 debugger/NMI fallback 的 nested #DB 同轮直接反射")
    if nested_npf_value_cache:
        problems.append("不得保留已量测为负收益的 nested NPF value cache")
    if not nested_vmcb02_merge_note:
        problems.append("必须保留 VMCB02 的上游 L0/L1 CPUID OR 合并说明")
    if not vmcb12_map_reuse:
        problems.append("必须保留单次 nested run、generation-checked 的 VMCB12 map reuse")
    if not re.search(r"trace_kvm_cpuid\(0, (?:index|kvm_ecx_read\(vcpu\))", added_text) or \
        ...
        problems.append("必须汇出并保留 kvm_cpuid tracepoint，以验证 fastpath 实际命中")
    added_cpuid_ops = re.findall(
        r"svm_(?:set|clr)_intercept\(svm, INTERCEPT_CPUID\);",
        added_text,
    )
    if len(added_cpuid_ops) != 5:
        problems.append("CPUID intercept 变更数量不符 Linux 7.2 policy")
    if re.search(
        r"\bnative_cpuid\b|guest_cpu_cap_has\(vcpu, X86_FEATURE_SVM\)",
        added_text,
    ):
        problems.append("不得因 guest 隐藏 SVM 而让 VMCB01 原生执行 CPUID")
    ...
    if direct_vmcb02_cpuid or cpl_cpuid_policy or re.search(
        ...
        problems.append("nested VMCB02 不得改写 CPUID intercept，必须保留 L0/L1 merged ownership")
    if "kvm_rax_read(vcpu) == 0" in added_text and "kvm_emulate_cpuid(vcpu)" in added_text:
        problems.append("不得在 svm_handle_exit 绕过 L1 的 nested CPUID exit")
    if "kvm_hv_hypercall_enabled(vcpu)" in added_text:
        problems.append("不得依赖 L2 Hyper-V hypercall state")
    if "svm_is_intercept" in added_text:
        problems.append("不得加入动态 CPUID intercept 探测")
    if problems:
        raise AppError(
            f"核心补丁 {patch.name} 的 CPUID intercept 路径不符合 Linux 7.2 policy："
            + "；".join(problems)
        )
    dynamic_markers = (
    ...
    if present:
        raise AppError(
            f"核心补丁 {patch.name} 仍包含动态 CPUID 切换："
            + ", ".join(present)
        )
...
def _queue_dkms_mok_enrollment(runner: Runner, purpose: str) -> None:
    if not MOK_CERTIFICATE.is_file() or not MOK_PRIVATE_KEY.is_file():
        raise AppError(
            "Secure Boot 已启用，但 Ubuntu 未建立 DKMS MOK 金钥；"
            "请确认 shim-signed、mokutil、openssl 与 sbsigntool 已安装。"
        )
    if _mok_enrolled(runner):
        return
    print(f"Secure Boot 需要信任用来签署{purpose}的 Ubuntu DKMS MOK 金钥。")
    print("请设定一组 8–16 字元的一次性 MOK 密码，重新开机时还要输入同一组密码。")
    ...
    if not pending.stdout.strip():
        raise AppError("MOK 登录要求没有写入 EFI；请确认系统以 UEFI 启动且 EFI variables 可写入。")
    print("MOK 登录要求已排程；请重新开机并在 MOK Manager 完成 Enroll MOK。")
...
def _ensure_kernel_mok(runner: Runner) -> None:
    ...
    if KERNEL_MOK_CERTIFICATE.exists() or KERNEL_MOK_PRIVATE_KEY.exists():
        raise AppError(
            f"专用核心签章金钥不完整；请先备份后移除 {KERNEL_MOK_DIRECTORY} 再重试。"
        )
...
def _queue_kernel_mok_enrollment(runner: Runner) -> None:
    ...
    if KERNEL_MOK_COMMON_NAME.lower() in (pending.stdout + pending.stderr).lower():
        print("专用核心开机 MOK 已在等待下次开机登录。")
        return
    print("Secure Boot 需要另外信任专用于 linux-tkg 开机映像的 MOK。")
    print("这与 DKMS/memflow 的 module-only MOK 不同；请设定一组 8–16 字元的一次性密码。")
    ...
    if not pending.stdout.strip():
        raise AppError("专用核心 MOK 登录要求没有写入 EFI。")
    print("专用核心 MOK 登录要求已排程。")
...
def _validate_tkg_boot_config(version: str) -> None:
    ...
    if not config.is_file():
        raise AppError(f"找不到自订核心设定档：{config}")
    ...
    if (
        ...
        raise AppError(
            f"{version} 没有启用 AppArmor LSM，会使 libvirt/AppArmor 维护失败；"
            "已中止切换到此核心。"
        )
...
def _ensure_nvidia_dkms_support(runner: Runner) -> bool:
    ...
    for package, version in drivers:
        ...
        if companion is None:
            raise AppError(
                f"已安装 NVIDIA 驱动 {version}，但缺少匹配的 {package}。"
                "自订核心将没有 NVIDIA 显示模组；请把相同版本的 companion DEB "
                "加入 offline/debs 后重新产生 Packages 与 manifest。"
            )
...
def _sign_tkg_dkms_modules(runner: Runner, version: str) -> bool:
    ...
    if not MOK_PRIVATE_KEY.is_file() or not MOK_CERTIFICATE.is_file():
        raise AppError("DKMS MOK 金钥不存在，无法签署自订核心的外部模组。")
    sign_file = Path("/usr/src") / f"linux-headers-{version}" / "scripts/sign-file"
    if not sign_file.is_file():
        raise AppError(f"找不到 {version} 的 scripts/sign-file，无法签署 DKMS 模组。")
    ...
    for image in images:
        ...
        if nvidia_required:
            ...
            if check.returncode != 0:
                raise AppError(
                    f"NVIDIA DKMS 没有为 {version} 产生 nvidia.ko；"
                    "不可重新开机进入此核心。"
                )
    if secure_boot and signed_any:
        _queue_dkms_mok_enrollment(runner, "自订核心的 DKMS 驱动")


def _sign_installed_tkg_kernels(runner: Runner) -> None:
    images = sorted(BOOT_DIR.glob("vmlinuz-*-tkg-*"))
    if not images:
        raise AppError("linux-tkg 套件已安装，但 /boot 中找不到自订核心映像。")
    for image in images:
        version = image.name.removeprefix("vmlinuz-")
...
def sign_custom_kernels(runner: Runner) -> None:
    ...
    if secure_boot:
        ...
        print("Secure Boot 未启用；自订核心映像不需要 MOK 签章。")
    ...
    print("自订 linux-tkg 核心与 DKMS 驱动已安装并验证。")
    if secure_boot and not _mok_enrolled(runner, KERNEL_MOK_CERTIFICATE):
        print("请重新开机并在 MOK Manager 完成 Enroll MOK 后，才能由 Secure Boot 启动。")
...
def build_kernel(runner: Runner, profile_key: str = "stable") -> None:
    ...
    if not linux.is_dir():
        if profile.test_release:
            raise AppError(
                f"找不到 Linux 7.2.2 测试核心离线 source：{linux}。"
                "请在准备部署包的连网 Ubuntu 上执行 tools/prepare_offline.sh，"
                "再把完整 offline 目录带到这台机器。"
            )
    ...
    if profile.test_release:
        ...
        if patched:
            print("已套用 Linux 7.2 的 NVIDIA DKMS strscpy 相容修补：")
            for path in patched:
                print(f"  - {path}")
    ...
    if len(image_debs) != 1:
        raise AppError("linux-tkg 产物无法唯一辨识非 debug 核心映像套件。")
    ...
    runner.run(["update-initramfs", "-u", "-k", "all"])
    runner.run(["update-grub"])
    print(f"已建置并安装 {profile.label}。稳定 Ubuntu 核心仍保留为回复入口。")
...
def cleanup_kernel_build(runner: Runner, profile_key: str = "test-7.2") -> None:
    ...
    if not existing:
        print(f"没有可清理的 {profile.label} 建置 source。")
        return
    print("将删除以下已完成建置的暂存 source/mirror（保留 DEBS 与已安装核心）：")
    for path in existing:
        print(f"  - {path}")
    if not prompt_yes_no("确认清理这些建置暂存", False):
        print("已取消清理。")
        return
    for path in existing:
        shutil.rmtree(path)
    runner.run(["apt-get", "clean"])
    print("核心建置暂存已清理；DEBS、DKMS、/boot 核心与离线 source 均保留。")
...
def install_memflow(runner: Runner) -> None:
    ...
    if loaded.returncode == 0:
        print("memflow 已安装并载入。")
        return
    detail = (loaded.stderr or loaded.stdout).strip()
    if not secure_boot:
        raise AppError(f"memflow 已由 DKMS 安装，但无法载入：\n{detail}")
    if not MOK_CERTIFICATE.is_file():
        raise AppError(
            "Secure Boot 已启用，但 Ubuntu 未建立 DKMS MOK 凭证；"
            "请确认 shim-signed、mokutil 与 openssl 已安装。"
        )
    test = runner.run(
    ...
    if "is not enrolled" not in (test.stdout + test.stderr).lower():
        raise AppError(f"memflow 的签署金钥已登录，但模组仍无法载入：\n{detail}")
    print("Secure Boot 正在阻挡 memflow；现在将登录 Ubuntu DKMS 的 MOK 凭证。")
    print("请设定一组 8–16 字元的一次性 MOK 密码，重新开机时还要输入同一组密码。")
    runner.run(["update-secureboot-policy", "--enroll-key"])
    pending = runner.run(
        ["mokutil", "--list-new"], check=False, capture=True,
    ...
    if not pending.stdout.strip():
        raise AppError("MOK 登录要求没有写入 EFI；请确认系统以 UEFI 启动且 EFI variables 可写入。")
    print("MOK 登录要求已排程；memflow DKMS 安装已完成。")
    print("请重新开机，在蓝色 MOK Manager 选 Enroll MOK → Continue → Yes，输入刚才的密码后再重新开机。")
    print("回到 Ubuntu 后再次选『安装 memflow』；已登录时会直接载入，不会重复要求 MOK。")
