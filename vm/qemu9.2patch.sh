...
get_pci_ids() {
    ...
    if ! command -v lspci &> /dev/null; then
        echo "错误: 需要安装pciutils (lspci命令)"
        exit 1
    fi
    
    # 检测CPU供应商
    IFS=':'
    cpu_vendor_arr=( $(cat /proc/cpuinfo | grep 'vendor_id' | uniq) )
    cpu_vendor="${cpu_vendor_arr[1]}"
    ...
    get_device_id() {
        ...
        # 尝试从lspci获取设备ID
        local device_id=$(lspci -nn | grep -i "$search_pattern" | grep "$vendor_id:" | \
                         head -1 | grep -o '\[....:....\]' | sed -e 's/\[//' -e 's/\]//' | \
                         cut -d':' -f2)
        ...
        if [ ! -z "$device_id" ]; then
            ...
            echo "$default_value"
            echo "警告: 未找到 $search_pattern，使用默认值: $default_value" >&2
        fi
    }
    
    # AMD平台设备
    if [ "$vendor_prefix" = "1022" ]; then
        lpc_1022=$(get_device_id "LPC Bridge" "1022" "790E")
        smbus_1022=$(get_device_id "SMBus Controller" "1022" "790B")
        hdaudio_1022=$(get_device_id "HD Audio Controller" "1022" "1637")
        hdaname_1022="Renoir HD Audio Controller"
        sata_1022=$(get_device_id "SATA Controller.*AHCI" "1022" "7901")
        rootport_1022=$(get_device_id "Renoir.*Function 0" "1022" "1448")
        xhci_1022=$(get_device_id "USB XHCI Controller" "1022" "7914")
        hostbridge_1022=$(get_device_id "PCIe GPP Bridge" "1022" "1633")
        ...
        rootport_8086="06BA"
        xhci_8086="06ED"
        hostbridge_8086="9B54"
        
    else  # Intel平台
        lpc_8086=$(get_device_id "ISA bridge" "8086" "068D")
        smbus_8086=$(get_device_id "SMBus Controller" "8086" "A3A3")
        hdaudio_8086=$(get_device_id "cAVS" "8086" "A3F0")
        hdaname_8086="Comet Lake PCH-V cAVS"
        sata_8086=$(get_device_id "SATA AHCI Controller" "8086" "06D2")
        rootport_8086=$(get_device_id "PCI Express Root Port" "8086" "06BA")
        xhci_8086=$(get_device_id "xHCI Host Controller" "8086" "06ED")
        hostbridge_8086=$(get_device_id "Host Bridge/DRAM Registers" "8086" "9B54")
...
if [[ -z "${KVMSPOOF_WORK_DIR:-}" ]]; then
    echo "错误：未设置 KVMSPOOF_WORK_DIR，拒绝克隆 QEMU 到临时目录。"
    exit 1
fi
WORK_DIR="$KVMSPOOF_WORK_DIR"
mkdir -p "$WORK_DIR"
cd "$WORK_DIR"
QEMU_SOURCE_BRANCH="stable-9.2"
QEMU_EXPECTED_VERSION="9.2"
QEMU_BACKUP_DIR="$WORK_DIR/qemubackup"
QEMU_DIR="$WORK_DIR/qemu"

VARS_TARGET=""

resolve_vars_file() {
    local runtime_root="${KVMSPOOF_RUNTIME_DIR:-/tmp/spoof}"
    local vars_dir="${KVMSPOOF_VARS_DIR:-$runtime_root}"
    local candidate

    for candidate in \
        "${VARS_FILE:-}" \
        "$vars_dir/vars.sh" \
        "$SCRIPT_DIR/KvmSPoof/vars.sh" \
        "$SCRIPT_DIR/vars.sh" \
        "${KVMSPOOF_STATE_FILE:-}" \
        "$runtime_root/vars.sh"
    do
        if [[ -n "$candidate" && -f "$candidate" ]]; then
            VARS_TARGET="$candidate"
            return 0
        fi
    done
    ...
    if [[ -n "${VARS_FILE:-}" ]]; then
        ...
        VARS_TARGET="$vars_dir/vars.sh"
    fi
    mkdir -p "$(dirname "$VARS_TARGET")" 2>/dev/null || true
    return 1
}

load_vm_defaults() {
    if resolve_vars_file; then
        # shellcheck disable=SC1090
        source "$VARS_TARGET" 2>/dev/null || true
        echo "已加载配置：$VARS_TARGET"
        echo "  硬盘 ${DISK_CAPACITY:-未设置}，内存 ${VM_MEMORY_MB:-未设置}MB，厂商 ${HDD_VENDOR_NAME:-未设置}"
    else
        echo "警告：未找到 vars.sh，硬盘厂商将回退为 QEMU，硬盘容量/内存使用默认值。"
    fi
}
...
normalize_vm_config() {
    ...
    case "$DISK_CAPACITY" in
        *[Gg][Bb]) DISK_CAPACITY="${DISK_CAPACITY%[Gg][Bb]}GB" ;;
        *[Gg]) DISK_CAPACITY="${DISK_CAPACITY%[Gg]}GB" ;;
        *[Tt][Bb]) DISK_CAPACITY="${DISK_CAPACITY%[Tt][Bb]}TB" ;;
        *[Tt]) DISK_CAPACITY="${DISK_CAPACITY%[Tt]}TB" ;;
        *[0-9]) DISK_CAPACITY="${DISK_CAPACITY}GB" ;;
    esac

    if [[ "$VM_MEMORY_MB" =~ ^[0-9]+$ ]] && [[ "$VM_MEMORY_MB" -gt 0 ]]; then
        VM_MEMORY_GB=$(((VM_MEMORY_MB + 1023) / 1024))
    else
        VM_MEMORY_MB=8192
        VM_MEMORY_GB=8
    fi
...
normalize_hdd_vendor_config() {
    case "${HDD_VENDOR_ID:-}" in
        wd)           : "${HDD_VENDOR_NAME:=Western Digital}" ;;
        seagate)      : "${HDD_VENDOR_NAME:=Seagate}" ;;
        samsung)      : "${HDD_VENDOR_NAME:=Samsung}" ;;
        toshiba)      : "${HDD_VENDOR_NAME:=Toshiba}" ;;
        sandisk)      : "${HDD_VENDOR_NAME:=SanDisk}" ;;
        micron)       : "${HDD_VENDOR_NAME:=Micron}" ;;
        kingston)     : "${HDD_VENDOR_NAME:=Kingston}" ;;
        intel)        : "${HDD_VENDOR_NAME:=Intel}" ;;
        corsair)      : "${HDD_VENDOR_NAME:=Corsair}" ;;
        crucial)      : "${HDD_VENDOR_NAME:=Crucial}" ;;
        adata)        : "${HDD_VENDOR_NAME:=ADATA}" ;;
        hitachi)      : "${HDD_VENDOR_NAME:=Hitachi}" ;;
        hynix)        : "${HDD_VENDOR_NAME:=SK Hynix}" ;;
        plextor)      : "${HDD_VENDOR_NAME:=Plextor}" ;;
        transcend)    : "${HDD_VENDOR_NAME:=Transcend}" ;;
        pny)          : "${HDD_VENDOR_NAME:=PNY}" ;;
        patriot)      : "${HDD_VENDOR_NAME:=Patriot}" ;;
        sabrent)      : "${HDD_VENDOR_NAME:=Sabrent}" ;;
        siliconpower) : "${HDD_VENDOR_NAME:=Silicon Power}" ;;
        teamgroup)    : "${HDD_VENDOR_NAME:=Team Group}" ;;
        "")           : "${HDD_VENDOR_NAME:=QEMU}" ;;
        *)            : "${HDD_VENDOR_NAME:=${HDD_VENDOR_ID^^}}" ;;
    esac

    : "${HDD_VENDOR_CHOICE:=${HDD_VENDOR_ID}:${HDD_VENDOR_NAME}}"
    export HDD_VENDOR_ID HDD_VENDOR_NAME HDD_VENDOR_CHOICE
}
...
upsert_var() {
  ...
  [[ -f "$file" ]] || : > "$file"
  local content
  content="$(grep -v "^${key}=" "$file" 2>/dev/null; printf '%s="%s"\n' "$key" "$value")"
  printf '%s\n' "$content" > "$file"
}
...
normalize_vm_config
normalize_hdd_vendor_config
HDD_BRAND_LABEL="${HDD_VENDOR_NAME:-${HDD_VENDOR_ID:-QEMU}}"
HDD_MODEL_LABEL="$HDD_BRAND_LABEL SSD $DISK_CAPACITY"
echo "✅ 已加载硬盘厂商：$HDD_BRAND_LABEL (${HDD_VENDOR_ID:-未设置})"
...
echo "当前系统PCI设备ID:"
if [[ "${cpu_vendor:1}" == "AuthenticAMD" ]]; then
    echo "lpc_1022=\"$lpc_1022\"        # FCH LPC Bridge"
    echo "smbus_1022=\"$smbus_1022\"      # FCH SMBus Controller"
    echo "hdaudio_1022=\"$hdaudio_1022\"    # Renoir HD Audio Controller"
    echo "hdaname_1022=\"$hdaname_1022\""
    echo "sata_1022=\"$sata_1022\"       # FCH SATA Controller [AHCI mode]"
    echo "rootport_1022=\"$rootport_1022\"  # Renoir Device 24: Function 0"
    echo "xhci_1022=\"$xhci_1022\"       # FCH USB XHCI Controller"
    echo "hostbridge_1022=\"$hostbridge_1022\" # Renoir PCIe GPP Bridge"
else
    echo "lpc_8086=\"$lpc_8086\"        # Comet Lake LPC Controller"
    echo "smbus_8086=\"$smbus_8086\"      # Comet Lake PCH-V SMBus Host Controller"
    echo "hdaudio_8086=\"$hdaudio_8086\"    # Comet Lake PCH-V cAVS"
    echo "hdaname_8086=\"$hdaname_8086\""
    echo "sata_8086=\"$sata_8086\"       # Comet Lake SATA AHCI Controller"
    echo "rootport_8086=\"$rootport_8086\"  # Comet Lake PCI Express Root Port #1"
    echo "xhci_8086=\"$xhci_8086\"       # Comet Lake USB 3.1 xHCI Host Controller"
    echo "hostbridge_8086=\"$hostbridge_8086\" # 10th Gen Core Processor Host Bridge/DRAM Registers"
fi
echo "=========================================="
...
cleanup_extra_qemu_bins() {
    local bin_dir="${QEMU_DEST:-/usr/local/bin}"
    local tool=""
    local removed=()
    local extra_qemu_bins=(
        elf2dmp
        qemu-edid
        qemu-ga
        qemu-img
    ...
    for tool in "${extra_qemu_bins[@]}"; do
        if [[ -e "$bin_dir/$tool" ]]; then
            sudo rm -f "$bin_dir/$tool" && removed+=("$tool")
        fi
    done

    if (( ${#removed[@]} > 0 )); then
        echo "Removed extra QEMU tools from $bin_dir:"
        printf '  %s\n' "${removed[@]}"
    fi
}

install_qemu_rom_data() {
    local src_dir="pc-bios"
    local data_dir="${QEMU_DATA_DIR:-/usr/local/share/qemu}"

    if [[ ! -d "$src_dir" ]]; then
        echo "Error: cannot find QEMU ROM data directory: $src_dir"
        exit 1
    fi

    sudo mkdir -p "$data_dir" || {
        echo "Error: failed to create QEMU data dir: $data_dir"
        exit 1
    }

    sudo cp -fr "$src_dir/." "$data_dir/." || {
        echo "Error: failed to copy QEMU ROM data to $data_dir"
        exit 1
    }

    echo "QEMU ROM data copied to $data_dir"
}

SSDT1_DSL=""
SSDT2_DSL=""

resolve_required_ssdt() {
    local name="$1"
    local var_name="$2"
    local src=""
    local candidate=""
    local candidates=(
        "${KVMSPOOF_RESOURCE_DIR:-}/$name"
        "${RESOURCE_DIR:-}/$name"
        "${SCRIPT_DIR:-}/$name"
        "${SCRIPT_DIR:-}/KvmSPoof/$name"
        "$(pwd)/$name"
    )
    ...
    printf -v "$var_name" '%s' "$src"
    echo "Using $name from $src"
}
...
compile_ssdt_table() {
    ...
    if [[ ! -f "$src" ]]; then
        echo "Error: cannot find $(basename "$src") at $src"
        exit 1
    fi
...
cleanup_qemu_ssdt() {
    rm -f "$WORK_DIR/qemu/ssdt1.aml" "$WORK_DIR/qemu/ssdt2.aml"
}
trap cleanup_qemu_ssdt EXIT

install_qemu_and_ssdt_tables() {
    local qemu_bin="build/qemu-system-x86_64"
    local aml_file=""

    if ! command -v iasl >/dev/null 2>&1; then
        echo "错误：iasl 未安装，无法编译 ssdt1.dsl / ssdt2.dsl"
        echo "Ubuntu/Debian: sudo apt install acpica-tools"
        echo "Arch/Manjaro:  sudo pacman -S acpica"
        exit 1
    fi

    compile_ssdt_table "$SSDT1_DSL" "ssdt1"
    compile_ssdt_table "$SSDT2_DSL" "ssdt2"

    for aml_file in ssdt1.aml ssdt2.aml; do
        if [[ ! -f "$aml_file" ]]; then
            echo "错误：$aml_file 未生成"
            exit 1
        fi
    done

    if [[ ! -f "$qemu_bin" ]]; then
        echo "错误：qemu-system-x86_64 未生成"
        ls -la build/
        exit 1
    fi

    sudo mkdir -p "$QEMU_DEST"
    sudo install -m 0755 "$qemu_bin" "$QEMU_DEST/qemu-system-x86_64"
    installed_qemu_version="$("$QEMU_DEST/qemu-system-x86_64" --version | head -n 1)"
    if [[ "$installed_qemu_version" != *"$QEMU_EXPECTED_VERSION"* ]]; then
        echo "Error: installed binary version mismatch: $installed_qemu_version"
        exit 1
    fi
    echo "Installed QEMU: $installed_qemu_version"
    sudo install -m 0644 "ssdt1.aml" "$QEMU_DEST/ssdt1.aml"
    sudo install -m 0644 "ssdt2.aml" "$QEMU_DEST/ssdt2.aml"
    install_qemu_rom_data
    if [[ "${KEEP_QEMU_TOOLS:-0}" != "1" ]]; then
        cleanup_extra_qemu_bins
    fi
    echo "$QEMU_DEST/qemu-system-x86_64"
    echo "$QEMU_DEST/ssdt1.aml"
    echo "$QEMU_DEST/ssdt2.aml"
    rm -f "ssdt1.aml" "ssdt2.aml"
    echo "Cleaned generated SSDT AML files from QEMU work dir"
    verify_qemu_rom_data
}
...
verify_qemu_rom_data() {
    ...
    for rom_file in kvmvapic.bin vgabios-stdvga.bin; do
    ...
    if (( ${#missing[@]} > 0 )); then
        echo "Error: QEMU ROM data files were not installed to $data_dir"
        printf '  missing: %s\n' "${missing[@]}"
        echo "Make sure QEMU pc-bios files were copied, or install qemu-system-data/seabios."
        exit 1
    fi

    echo "QEMU ROM data installed: $data_dir"
}
...
if [[ -d "$QEMU_BACKUP_DIR" ]]; then
  source_branch="$(git -C "$QEMU_BACKUP_DIR" symbolic-ref --quiet --short HEAD 2>/dev/null || true)"
  if [[ ! -f "$QEMU_BACKUP_DIR/configure" || "$source_branch" != "$QEMU_SOURCE_BRANCH" ]]; then
    echo "Existing QEMU source is incomplete or on branch '${source_branch:-unknown}', expected $QEMU_SOURCE_BRANCH. Recloning..."
    rm -rf "$QEMU_BACKUP_DIR"
  fi
fi

if [[ ! -d "$QEMU_BACKUP_DIR" ]]; then
  echo "Cloning QEMU $QEMU_SOURCE_BRANCH into $QEMU_BACKUP_DIR..."
  git clone --single-branch --branch "$QEMU_SOURCE_BRANCH" https://github.com/qemu/qemu.git "$QEMU_BACKUP_DIR" || {
    echo "Error: failed to clone QEMU $QEMU_SOURCE_BRANCH"
    exit 1
  }
else
  echo "Verified QEMU source branch: $QEMU_SOURCE_BRANCH"
fi
...
if [[ -d "$QEMU_DIR" ]]; then
  echo "Removing previous QEMU work tree: $QEMU_DIR"
  rm -rf "$QEMU_DIR"
fi
mkdir -p "$QEMU_DIR"
cp -a "$QEMU_BACKUP_DIR/." "$QEMU_DIR/" || {
  echo "Error: failed to create clean QEMU work tree"
  exit 1
}
if [[ ! -f "$QEMU_DIR/configure" ]]; then
  echo "错误：QEMU 源码不完整，缺少 qemu/configure"
  exit 1
fi
resolve_required_ssdt "ssdt1.dsl" SSDT1_DSL
resolve_required_ssdt "ssdt2.dsl" SSDT2_DSL
#cp -f *.aml qemu/.
...
mem_size_expr="${VM_MEMORY_MB} * MiB"
echo "    m->smbios_memory_device_size = ${mem_size_expr};"
sed -i "$file_pcq35" -Ee "/    m->alias = \"q35\";/a\    m->smbios_memory_device_size = ${mem_size_expr};"
sed -i "$file_pcq35" -Ee "s/smbios_memory_device_size = 16([[:space:]]*[,;])/smbios_memory_device_size = ${mem_size_expr}\1/"
sed -i "$file_pcq35" -Ee "s/smbios_memory_device_size = 8([[:space:]]*\*[[:space:]]*GiB[[:space:]]*;)/smbios_memory_device_size = ${mem_size_expr};/"
sed -i "$file_pcq35" -Ee "s/smbios_memory_device_size = 8([[:space:]]*[,;])/smbios_memory_device_size = ${mem_size_expr}\1/"
...
echo "  $file_ctrl"
get_new_string 4 1
echo "QEMU NVMe Ctrl                                    -> $HDD_BRAND_LABEL NVMe Ctrl"
sed -i "$file_ctrl" -Ee "s/QEMU NVMe Ctrl/$HDD_BRAND_LABEL NVMe Ctrl/"
...
echo "QEMU MPT Fusion                                   -> $HDD_BRAND_LABEL MPT Fusion"
echo "\"QEMU\"                                            -> \"$HDD_BRAND_LABEL\""
echo "0000111122223333                                  -> $serial"
sed -i "$file_mptconfig" -Ee "s/QEMU MPT Fusion/$HDD_BRAND_LABEL MPT Fusion/"
sed -i "$file_mptconfig" -Ee "s/\"QEMU\"/\"$HDD_BRAND_LABEL\"/"
...
sed -i "$file_scsidisk" -Ee "s/\"QEMU\"/\"$HDD_BRAND_LABEL\"/"
sed -i "$file_scsidisk" -Ee "s/\"QEMU HARDDISK\"/\"$HDD_MODEL_LABEL\"/"
...
serial=$(get_random_serial 16)
echo "\"QEMU\",                                           -> \"$HDD_BRAND_LABEL\","
echo "QEMU USB HARDDRIVE                                -> $HDD_BRAND_LABEL USB SSD $DISK_CAPACITY"
echo "\"1\"                                               -> \"$serial\""
echo "_desc   = \"QEMU USB MSD\"                          -> _desc   = \"$HDD_BRAND_LABEL USB SSD $DISK_CAPACITY\""
sed -i "$file_devstorage" -Ee "s/\"QEMU\",/\"$HDD_BRAND_LABEL\",/"
sed -i "$file_devstorage" -Ee "s/QEMU USB HARDDRIVE/$HDD_BRAND_LABEL USB SSD $DISK_CAPACITY/"
sed -i "$file_devstorage" -Ee "s/\"1\"/\"$serial\"/"
sed -i "$file_devstorage" -Ee "s/_desc   = \"QEMU USB MSD\"/_desc   = \"$HDD_BRAND_LABEL USB SSD $DISK_CAPACITY\"/"

echo "  $file_devuas"
get_new_string $(shuf -i 5-7 -n 1) 3
number=$(get_random_dec 10)
echo "\"QEMU\",                                           -> \"$HDD_BRAND_LABEL\","
echo "USB Attached SCSI HBA                             -> $HDD_BRAND_LABEL USB Attached SCSI HBA"
echo "\"27842\"                                           -> \"$number\""
sed -i "$file_devuas" -Ee "s/\"QEMU\",/\"$HDD_BRAND_LABEL\",/"
sed -i "$file_devuas" -Ee "s/USB Attached SCSI HBA/$HDD_BRAND_LABEL USB Attached SCSI HBA/"
...
./configure --target-list=x86_64-softmmu || {
  echo "错误：QEMU configure 失败"
  exit 1
}
cd build
make -j$(nproc) || {
  echo "错误：QEMU make 编译失败"
  exit 1
}

# 编译并安装 QEMU 与 SSDT ACPI 表文件
cd ..
BUILT_QEMU_VERSION="$(./build/qemu-system-x86_64 --version | head -n 1)"
if [[ "$BUILT_QEMU_VERSION" != *"$QEMU_EXPECTED_VERSION"* ]]; then
  echo "Error: built binary version mismatch: $BUILT_QEMU_VERSION"
  exit 1
fi
echo "Built QEMU: $BUILT_QEMU_VERSION"
install_qemu_and_ssdt_tables

if [[ -n "$VARS_TARGET" ]]; then
  upsert_var HDD_VENDOR_COMPILED_ID    "${HDD_VENDOR_ID:-}"   "$VARS_TARGET"
  upsert_var HDD_VENDOR_COMPILED_NAME  "$HDD_BRAND_LABEL"     "$VARS_TARGET"
  upsert_var HDD_VENDOR_COMPILED_MODEL "$HDD_MODEL_LABEL"     "$VARS_TARGET"
  upsert_var HDD_VENDOR_COMPILED_AT    "$(date '+%Y-%m-%d %H:%M:%S')" "$VARS_TARGET"
  echo "已记录编译厂商到 $VARS_TARGET："
  echo "  HDD_VENDOR_COMPILED_ID=\"${HDD_VENDOR_ID:-}\""
  echo "  HDD_VENDOR_COMPILED_NAME=\"$HDD_BRAND_LABEL\""
  echo "  HDD_VENDOR_COMPILED_MODEL=\"$HDD_MODEL_LABEL\""
fi
