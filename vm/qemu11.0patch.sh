...
if [[ ! -f vars.sh ]]; then
  ...
  source vars.sh
  if [[ -z "${device:-}" ]]; then device=$(( ($(date +"%-d") + $(date +"%-m"))*100 + $(date +"%-d") * $(date +"%-m") )); fi
  if [[ -z "${vendor:-}" ]]; then vendor=$(( device + ((RANDOM%768)+256) )); fi
  if [[ -z "${xhci:-}" ]]; then xhci=$(( 49152 - device - ((RANDOM%768)+256) )); fi
  if [[ -z "${cpu:-}" ]]; then cpu=$(( 16384 + ((RANDOM%1024)*16) )); fi
  if [[ -z "${virtio:-}" ]]; then virtio=$(( 49152 + device )); fi
  if [[ -z "${edk2bridge_1022:-}" ]]; then edk2bridge_1022="$pcibridge_1022"; fi
  if [[ -z "${edk2bridge_8086:-}" ]]; then edk2bridge_8086="$pcibridge_8086"; fi
  echo "device=\"$device\""                  >  vars.sh
  echo "vendor=\"$vendor\""                  >> vars.sh
  echo "xhci=\"$xhci\""                      >> vars.sh
  echo "cpu=\"$cpu\""                        >> vars.sh
...
normalize_vm_config() {
  ...
  export DISK_CAPACITY VM_MEMORY_MB VM_MEMORY_GB
}
...
normalize_hdd_vendor_config() {
  ...
  export HDD_VENDOR_ID HDD_VENDOR_NAME HDD_VENDOR_CHOICE HDD_BRAND_LABEL HDD_MODEL_LABEL
}

persist_vm_config_to_vars() {
  [[ -f vars.sh ]] || return 0

  {
    echo "USER_CAPACITY=\"${USER_CAPACITY:-}\""
    echo "DISK_CAPACITY=\"$DISK_CAPACITY\""
    echo "USER_MEM_SLOT=\"${USER_MEM_SLOT:-${VM_MEMORY_GB:-}}\""
    echo "VM_MEMORY_GB=\"$VM_MEMORY_GB\""
    echo "VM_MEMORY_MB=\"$VM_MEMORY_MB\""
    echo "HDD_VENDOR_ID=\"${HDD_VENDOR_ID:-}\""
    echo "HDD_VENDOR_NAME=\"$HDD_VENDOR_NAME\""
    echo "HDD_VENDOR_CHOICE=\"$HDD_VENDOR_CHOICE\""
    echo "HDD_VENDOR_USED_IDS=\"${HDD_VENDOR_USED_IDS:-}\""
  } >> vars.sh
}
...
normalize_hdd_vendor_config
persist_vm_config_to_vars
echo "Loaded VM config: disk=$DISK_CAPACITY, memory=${VM_MEMORY_MB}MB, hdd=$HDD_BRAND_LABEL"

QEMU_SOURCE_BRANCH="stable-11.0"
...
cp -f "$SSDT1_DSL" "$file_ssdt1" || {
  echo "Error: failed to copy ssdt1.dsl to $file_ssdt1"
  exit 1
}
cp -f "$SSDT2_DSL" "$file_ssdt2" || {
  echo "Error: failed to copy ssdt2.dsl to $file_ssdt2"
  exit 1
}
#cp -f *.aml qemu
...
sed -i "$file_pcq35" -Ee "/    pc_q35_machine_options\(m\);/a\    m->smbios_memory_device_size = ${mem_size_expr};"
...
./configure --target-list=x86_64-softmmu || {
  echo "Error: QEMU configure failed"
  exit 1
}
cd build
make -j"$(nproc)" || {
  echo "Error: QEMU make failed"
  exit 1
}

if ! command -v iasl >/dev/null 2>&1; then
  echo "Error: iasl is not installed, cannot compile ssdt1.dsl / ssdt2.dsl"
  echo "Ubuntu/Debian: sudo apt install acpica-tools"
  echo "Arch/Manjaro:  sudo pacman -S acpica"
  exit 1
fi

compile_ssdt_table "$file_ssdt1" "ssdt1"
compile_ssdt_table "$file_ssdt2" "ssdt2"

for aml_file in ssdt1.aml ssdt2.aml; do
  if [[ ! -f "$aml_file" ]]; then
    echo "Error: $aml_file was not generated"
    exit 1
  fi
done

sudo mkdir -p "$QEMU_DEST"
sudo install -m 0755 "$(pwd)/qemu-system-x86_64" "$QEMU_DEST/qemu-system-x86_64"
