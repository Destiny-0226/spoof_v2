...
int vmx_get_msr(struct kvm_vcpu *vcpu, struct msr_data *msr_info)
{
	...
	case MSR_IA32_FEAT_CTL: {
		u64 data = FEAT_CTL_LOCKED;

		if (guest_cpu_cap_has(vcpu, X86_FEATURE_VMX))
			data |= FEAT_CTL_VMX_ENABLED_INSIDE_SMX |
				FEAT_CTL_VMX_ENABLED_OUTSIDE_SMX;
		if (guest_cpu_cap_has(vcpu, X86_FEATURE_SGX))
			data |= FEAT_CTL_SGX_ENABLED;
		if (guest_cpu_cap_has(vcpu, X86_FEATURE_SGX_LC))
			data |= FEAT_CTL_SGX_LC_ENABLED;
		if (vmx->msr_ia32_feature_control & FEAT_CTL_LMCE_ENABLED)
			data |= FEAT_CTL_LMCE_ENABLED;
		msr_info->data = data;
		break;
	}
