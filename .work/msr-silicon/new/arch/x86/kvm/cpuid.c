...
/*
 * Host-fill clock/power identity.  0x06 keeps HWP but not HFI/ITD.
 * 0x1A is native core type (P=0x40 / E=0x20); CPU-Z still used ACPI*800
 * BCLK while 0x15/0x16 were filled and 0x1A was zero.
 */
static bool ovo_cpuid_host_leaf(struct kvm_vcpu *vcpu, u32 function)
{
	struct kvm_cpuid_entry2 *limit;
	...
	limit = kvm_find_cpuid_entry_emulated(vcpu, 0,
			KVM_CPUID_INDEX_NOT_SIGNIFICANT);
	return limit && function <= limit->eax;
}
	       ...
	       u32 *ecx, u32 *edx, bool exact_only)
{
	...
	if (ovo_cpuid_host_leaf(vcpu, function)) {
		cpuid_count(function, index, eax, ebx, ecx, edx);
		if (function == 0x06) {
			*eax &= ~((1u << 19) | (1u << 23));
			*edx &= 0x7f;
		}
		kvm_trace_cpuid(orig_function, index, *eax, *ebx, *ecx, *edx,
				true, false);
		return true;
	}
