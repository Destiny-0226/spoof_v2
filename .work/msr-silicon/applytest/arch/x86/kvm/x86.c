...
/*
 * Guest MSR contract:
 *  1) CPUID-hidden named VT/SVM/HFI MSRs #GP.
 *  2) vCPU-state MSRs stay on KVM (TSC, APIC, EFER, MTRR, SPEC_CTRL, vPMU…).
 *  3) Everything else follows host rdmsrq_safe: missing => #GP, present =>
 *     host value or per-vCPU overlay. Writes never wrmsr the host.
 *  4) Host control-flow MSRs (LBR/DS) exist but return 0/overlay, not host IPs.
 */
#define OVO_MSR_IA32_DEBUG_INTERFACE	0x00000c80u

static bool ovo_msr_hidden_by_cpuid(struct kvm_vcpu *vcpu, u32 msr)
{
	if (!guest_cpu_cap_has(vcpu, X86_FEATURE_VMX) &&
	    msr >= MSR_IA32_VMX_BASIC && msr <= MSR_IA32_VMX_PROCBASED_CTLS3)
		return true;

	if (!guest_cpu_cap_has(vcpu, X86_FEATURE_SVM) &&
	    (msr == MSR_VM_CR || msr == MSR_VM_IGNNE ||
	     msr == MSR_VM_HSAVE_PA || msr == MSR_AMD64_SVM_AVIC_DOORBELL))
		return true;

	if (msr == MSR_IA32_HW_FEEDBACK_PTR ||
	    msr == MSR_IA32_HW_FEEDBACK_CONFIG ||
	    msr == 0x17d2 || msr == 0x17d4 || msr == 0x17da)
		return true;

	return false;
}

static bool ovo_msr_is_virtualized(u32 msr)
{
	switch (msr) {
	case MSR_EFER:
	case MSR_STAR:
...
static bool ovo_msr_hide_host_payload(u32 msr)
{
	if (msr == MSR_IA32_DS_AREA || msr == MSR_LBR_SELECT || msr == MSR_LBR_TOS)
		return true;
	if (msr == MSR_IA32_LASTBRANCHFROMIP || msr == MSR_IA32_LASTBRANCHTOIP ||
	    msr == MSR_IA32_LASTINTFROMIP || msr == MSR_IA32_LASTINTTOIP)
		return true;
	if (msr >= MSR_LBR_CORE_FROM && msr <= MSR_LBR_CORE_FROM + 7)
		return true;
	if (msr >= MSR_LBR_CORE_TO && msr <= MSR_LBR_CORE_TO + 7)
		return true;
	if (msr >= MSR_LBR_NHM_FROM && msr <= MSR_LBR_NHM_FROM + 31)
		return true;
	if (msr >= MSR_LBR_NHM_TO && msr <= MSR_LBR_NHM_TO + 31)
		return true;
	if (msr >= MSR_LBR_INFO_0 && msr <= MSR_LBR_INFO_0 + 31)
		return true;
	return false;
}

static bool ovo_msr_write_is_ro(u32 msr)
{
	switch (msr) {
	...
	case MSR_SMI_COUNT:
	case OVO_MSR_IA32_DEBUG_INTERFACE:
	case 0x0db0:
	case 0x0db1:
	case 0x0db2:
	case 0x0982:
...
static bool ovo_msr_write_discard(u32 msr)
{
	return msr == MSR_IA32_THERM_STATUS || msr == MSR_HWP_STATUS;
}

static bool ovo_msr_overlay_lookup(struct kvm_vcpu *vcpu, u32 msr, u64 *data)
{
	u16 i, n = vcpu->arch.ovo_msr_overlay_n;

	for (i = 0; i < n; i++) {
		if (vcpu->arch.ovo_msr_overlay_idx[i] == msr) {
			*data = vcpu->arch.ovo_msr_overlay_val[i];
			return true;
		}
	}
	return false;
...
static int ovo_msr_overlay_store(struct kvm_vcpu *vcpu, u32 msr, u64 data)
{
	u16 i, n = vcpu->arch.ovo_msr_overlay_n;

	for (i = 0; i < n; i++) {
		if (vcpu->arch.ovo_msr_overlay_idx[i] == msr) {
			vcpu->arch.ovo_msr_overlay_val[i] = data;
			return 0;
		}
	}
	if (n >= ARRAY_SIZE(vcpu->arch.ovo_msr_overlay_idx))
		return KVM_MSR_RET_UNSUPPORTED;
	vcpu->arch.ovo_msr_overlay_idx[n] = msr;
	vcpu->arch.ovo_msr_overlay_val[n] = data;
	vcpu->arch.ovo_msr_overlay_n = n + 1;
	return 0;
}

static int ovo_msr_guest_read(struct kvm_vcpu *vcpu, struct msr_data *msr_info)
{
	u64 host_val;
	...
	if (msr_info->host_initiated)
		return KVM_MSR_RET_UNSUPPORTED;
	if (ovo_msr_hidden_by_cpuid(vcpu, msr))
		return KVM_MSR_RET_UNSUPPORTED;
	if (ovo_msr_overlay_lookup(vcpu, msr, &msr_info->data))
		return 0;
	if (msr == OVO_MSR_IA32_DEBUG_INTERFACE) {
		if (rdmsrq_safe(msr, &host_val))
			return KVM_MSR_RET_UNSUPPORTED;
		msr_info->data = BIT_ULL(30);
		return 0;
	}
	if (ovo_msr_hide_host_payload(msr)) {
		if (rdmsrq_safe(msr, &host_val))
			return KVM_MSR_RET_UNSUPPORTED;
		msr_info->data = 0;
		return 0;
	}
	if (rdmsrq_safe(msr, &msr_info->data))
		return KVM_MSR_RET_UNSUPPORTED;
	return 0;
}

static int ovo_msr_guest_write(struct kvm_vcpu *vcpu, struct msr_data *msr_info)
{
	u64 host_val;
	u32 msr = msr_info->index;
	...
	if (ovo_msr_hidden_by_cpuid(vcpu, msr))
		return KVM_MSR_RET_UNSUPPORTED;
	if (ovo_msr_write_is_ro(msr))
		return KVM_MSR_RET_UNSUPPORTED;
	if (rdmsrq_safe(msr, &host_val))
		return KVM_MSR_RET_UNSUPPORTED;
	if (ovo_msr_write_discard(msr))
		return 0;
	return ovo_msr_overlay_store(vcpu, msr, msr_info->data);
}
...
int kvm_set_msr_common(struct kvm_vcpu *vcpu, struct msr_data *msr_info)
{
	...
	if (!msr_info->host_initiated && ovo_msr_hidden_by_cpuid(vcpu, msr))
		return KVM_MSR_RET_UNSUPPORTED;
	if (!msr_info->host_initiated && !ovo_msr_is_virtualized(msr)) {
		if (kvm_pmu_is_valid_msr(vcpu, msr))
			return kvm_pmu_set_msr(vcpu, msr_info);
		return ovo_msr_guest_write(vcpu, msr_info);
	}
...
{
	if (!msr_info->host_initiated &&
	    ovo_msr_hidden_by_cpuid(vcpu, msr_info->index))
		return KVM_MSR_RET_UNSUPPORTED;
	if (!msr_info->host_initiated &&
	    !ovo_msr_is_virtualized(msr_info->index)) {
		if (kvm_pmu_is_valid_msr(vcpu, msr_info->index))
			return kvm_pmu_get_msr(vcpu, msr_info);
		return ovo_msr_guest_read(vcpu, msr_info);
	}
